function PayGapByJob2017() {
  ...
  this.draw = function() {
    ...
    for (i = 0; i < this.data.getRowCount(); i++) {
      ...
      let x = map(propFemale[i],propFemaleMin,propFemaleMax,0+this.pad,width-this.pad);
      let y = map(payGap[i],payGapMax,payGapMin,0 +this.pad,height-this.pad);
      let size = map(numJobs[i],numJobsMin,numJobsMax,0,30);
      ellipse(
        /// ???
          x,
          y,
  ...
  this.addAxes = function () {
    stroke(200);
    fill('black');  
    text(`${this.name}`,width/2.5,this.pad/2);
    // Add vertical line.
    line(width / 2,
         0 + this.pad,
         width / 2,
    ...
    // Add horizontal line.
    text(`${this.id}`,this.pad/2,this.pad+height/2);
      text(`Male paid more`,this.pad/2,this.pad+height/4);
      text(`Female paid more`,this.pad/2,this.pad+height/1.35);
