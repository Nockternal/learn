function Nutrients() {
  ...
  this.name = 'Nutrients Intake 1974-2018';
  ...
  this.title = 'Nutrients: timeline 1974 - 2018';

    // Names for each axis.
  this.xAxisLabel = 'year';
  this.yAxisLabel = 'Volume';
  this.colors = [];
  ...
  this.preload = function() {
    var self = this;
    this.data = loadTable(
      './data/food/UK-foodIntake.csv', 'csv', 'header',
  ...
  this.setup = function() {
    ...
    for (var i = 0; i < this.data.getRowCount(); i++)
    {
      ...
      {
        if (row.getNum(j) > this.maxValue) {
          this.maxValue = row.getNum(j)
        }
      }
    }
    this.maxPercentage = Math.ceil(this.maxValue/50)*50
      ...
      for (var j = 1; j < this.data.getColumnCount(); j++) 
      {
        ...
        if (previous != null) 
        {
          ...
          line(this.mapYearToWidth(previous.year),
              this.mapPayGapToHeight(previous.percentages),
              this.mapYearToWidth(current.year),
              this.mapPayGapToHeight(current.percentages));
